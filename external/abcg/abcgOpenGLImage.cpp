...
GLuint loadOpenGLTexture(OpenGLTextureCreateInfo const &createInfo) {
  auto img{abcg::loadImage(createInfo.path)};
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, gsl::narrow<GLint>(internalFormat), img.width,
               img.height, 0, format, GL_UNSIGNED_BYTE, img.data.data());
...
GLuint loadOpenGLCubemap(OpenGLCubemapCreateInfo const &createInfo) {
  ...
  for (auto &&[index, path] : iter::enumerate(createInfo.paths)) {
    auto img{abcg::loadImage(path, 3)};
    ...
    glTexImage2D(target, 0, GL_RGB, img.width, img.height, 0, GL_RGB,
